...
public class SleepService {
    ...
    @Transactional
    public SleepResponse saveOrUpdate(Long userId, SleepRequest request) {
        if (userId == null || request.getDate() == null) {
            throw new IllegalArgumentException("UserId and Date must not be null");
        }

        Optional<SleepData> existing = repository.findByUserIdAndDate(userId, request.getDate());
        SleepData entity;
