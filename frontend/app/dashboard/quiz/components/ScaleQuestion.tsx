        ...
        <div key={i} className="flex flex-col items-center space-y-2">
          <button
            ...
            className={`
              ...
              ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:transform hover:scale-105'}
            `}
          >
            {i}
          </button>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <div className="flex justify-between mt-4 px-4 text-sm text-gray-500">
          <span>{quiz.scaleLabels?.[scaleMin.toString()] || `${scaleMin}`}</span>
          <span>{quiz.scaleLabels?.[scaleMax.toString()] || `${scaleMax}`}</span>
        </div>
      </div>
      ...
      {selectedValue && (
        <div className="text-center">
          <span className="inline-block bg-blue-100 text-blue-800 text-sm px-3 py-1 rounded-full">
            Selected: {selectedValue} - {quiz.scaleLabels?.[selectedValue] || selectedValue}
          </span>
        </div>
      )}
    </div>
