...
import { cachedBreathingApi, breathingCache, formatDateForApi as breathingFormatDate } from '../mindfulness/api/breathingApi';
...
const ActivityList = () => {
    ...
    const checkBreathingStatus = async (): Promise<boolean> => {
        try {
            ...
            const response = await fetch(`http://localhost:8080/api/v1/content/breathing/session?date=${today}`, {
                method: 'GET',
                credentials: 'include',
                headers: {
    ...
    return (
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
            ...
            <p className="text-sm text-gray-600 mb-4">
                Let's make today amazing together! {completedCount > 0 && `Great job on completing ${completedCount} activities! 🌟`}
            </p>
