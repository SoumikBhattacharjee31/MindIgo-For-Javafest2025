  ...
  return (
    <div
      className={`flex items-center justify-between p-3 bg-gradient-to-r rounded-lg transition-all cursor-pointer ${
        isCompleted ? "from-green-50 to-emerald-50 opacity-75" : getTypeColor()
      }`}
      onClick={onClick}
    >
      <div className="flex items-center gap-3 flex-1">
        <div
          className={`${
            isCompleted ? "text-green-600" : getTypeColor().split(" ").pop()
          }`}
        >
          {getIcon()}
        </div>
        <div className="flex-1">
          ...
          {time && <p className="text-gray-600 text-xs">{time}</p>}
          <p
