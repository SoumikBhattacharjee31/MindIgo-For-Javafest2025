import { Moon, Wind, Smile, Heart, Star } from "lucide-react";

import metricData from "@/app/dashboard/mock/metric_data.json";
import MetricCard from "./MetricCard";
import SleepMetricCard from "./SleepMetricCard";
import MoodMetricCard from "./MoodMetricCard";
    ...
    <div>
      <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
        ...
        <span className="text-sm font-normal text-gray-600">
          Track more to unlock magical insights 🎉
        </span>
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {metricData.map((metric, index) =>
          metric.id === "sleep" ? (
            <SleepMetricCard key={index} />
          ) : metric.id === "mood" ? (
            <MoodMetricCard key={index} />
          ) : (
            <MetricCard
              key={index}
              title={metric.title}
