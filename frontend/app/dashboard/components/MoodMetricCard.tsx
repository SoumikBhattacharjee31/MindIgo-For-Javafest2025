...
const MoodMetricCard: React.FC = () => {
  ...
  const yesterday = formatDateForApi(
  ...
  const loadMoodData = async () => {
    try {
      ...
      const yesterdayEntry =
        moodData.find((entry) => entry.date === yesterday) || null;
      ...
      >
        ...
        {loading ? (
            ...
            <div className="flex items-center gap-3 mt-1 mb-3">
              <div className="flex items-center gap-2">
                <span className="text-2xl">
                  {getMoodDetails(todayMood.mood).emoji}
                </span>
              ...
              {(() => {
                ...
                return comparison ? (
                  <span
                    className={`text-sm font-medium px-2 py-1 rounded-full ${comparison.color}`}
                  >
                    {comparison.text}
                  </span>
                ) : null;
          ...
          <>
            {/* No mood data */}
            <h3 className="text-gray-600 text-sm font-medium">Today's Mood</h3>
            <p className="text-2xl font-bold text-gray-400 mt-1 mb-3">
              Not tracked
            </p>
