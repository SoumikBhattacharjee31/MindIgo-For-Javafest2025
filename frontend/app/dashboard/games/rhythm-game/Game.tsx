...
class EnhancedRhythmScene extends Phaser.Scene {
  ...
  init(data: any) {
    ...
    this.beatmap = generateMockBeatmap(
  ...
  createAudioFromLoaded() {
    try {
      ...
      console.warn(
        "Phaser audio failed to load, game may not function correctly.",
        e
      );
      (this as any).music = {
        seek: 0,
        ...
        isPlaying: false,
        play: () => {
          console.warn("Mock audio play called.");
          (this as any).music.isPlaying = true;
        },
        pause: () => {
          (this as any).music.isPlaying = false;
        },
        resume: () => {
          (this as any).music.isPlaying = true;
        },
  ...
  stopGame() {
    ...
    const ev = new CustomEvent("rhythm:stopped", {
      detail: {
  ...
  update() {
    if (!this.gameStarted || this.gameEnded || this.gameStopped || !this.music)
      return;
    ...
    >
      <div className="text-center mb-8">
        <motion.h1
          ...
          className="text-5xl font-light mb-4 bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-sky-500"
        >
          Calm Rhythm
        </motion.h1>
        <motion.p
        ...
        >
          Choose your song and find your rhythm
        </motion.p>
      </div>
      ...
      <motion.div
        ...
        className="bg-white/70 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white/50 mb-6"
      >
        <h2 className="text-2xl font-semibold text-slate-800 mb-6 flex items-center gap-3">
          <Music className="w-6 h-6 text-indigo-500" /> Song Library
        </h2>
            ...
            <motion.button
              ...
              className={`p-6 rounded-2xl border-2 transition-all duration-300 text-left ${
                selectedSong.id === song.id
                  ? `border-indigo-400 bg-gradient-to-r ${song.color} text-white shadow-lg`
                  : "border-slate-200 bg-white/50 hover:border-indigo-300 hover:bg-white/70"
              }`}
            >
        ...
        <motion.div
          ...
          className="mt-6 p-6 rounded-2xl bg-gradient-to-r from-slate-50 to-slate-100 border border-slate-200"
        >
          <h3 className="text-xl font-semibold text-slate-800 mb-2">
            Now Playing Preview
          </h3>
          <div
            className={`text-2xl font-bold bg-gradient-to-r ${selectedSong.color} bg-clip-text text-transparent mb-2`}
          >
            {selectedSong.title}
          </div>
          ...
          <div className="flex gap-6 text-sm text-slate-600">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                Duration: {Math.floor(selectedSong.duration / 60)}:
      ...
      >
        <button
          ...
          className="flex items-center gap-3 bg-gradient-to-r from-slate-500 to-slate-600 text-white font-semibold py-3 px-8 rounded-xl hover:from-slate-600 hover:to-slate-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
        >
          <ArrowLeft className="w-5 h-5" /> Back to Games
        </button>
        <button
          onClick={() => onSongSelect(selectedSong)}
          className={`flex items-center gap-3 bg-gradient-to-r ${selectedSong.color} text-white font-semibold py-3 px-8 rounded-xl hover:shadow-xl transition-all duration-200 transform hover:scale-105 shadow-lg`}
        >
      ...
      >
        <p className="text-slate-500 text-sm">
          Use{" "}
          <kbd className="bg-slate-200 px-2 py-1 rounded text-slate-700 font-mono">
            A
          </kbd>{" "}
          <kbd className="bg-slate-200 px-2 py-1 rounded text-slate-700 font-mono mx-1">
            S
          </kbd>{" "}
          <kbd className="bg-slate-200 px-2 py-1 rounded text-slate-700 font-mono">
            D
...
function GameInterface({
  ...
  const [gameState, setGameState] = useState<"playing" | "stopped" | "ended">(
    "playing"
  );
  ...
  const handleVolumeChange = useCallback((newVolume: number) => {
    setVolume(newVolume);
    const event = new CustomEvent("rhythm:volume", {
  ...
  const handleGameControl = useCallback(
    (action: string) => {
      ...
      if (action === "stop") {
        const ev = new CustomEvent("rhythm:control", {
          detail: { action: "stop" },
        });
        window.dispatchEvent(ev);
      }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <motion.div
        initial={{ y: -20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        className="mb-6 flex items-center justify-between bg-white/70 backdrop-blur-md rounded-2xl p-4 shadow-lg border border-white/50"
      >
        <div className="flex items-center gap-4">
          <button
            onClick={handleBackToMenu}
            className="flex items-center gap-2 bg-gradient-to-r from-slate-500 to-slate-600 text-white font-medium py-2 px-4 rounded-xl hover:from-slate-600 hover:to-slate-700 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 text-sm"
          >
            <ArrowLeft className="w-4 h-4" /> Menu
          </button>
          <button
            onClick={onBackToGames}
            className="flex items-center gap-2 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-medium py-2 px-4 rounded-xl hover:from-indigo-600 hover:to-indigo-700 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 text-sm"
          >
        ...
        <div
          className={`text-center px-4 py-2 rounded-xl bg-gradient-to-r ${selectedSong.color} text-white shadow-md`}
        >
          ...
          <div className="flex gap-2">
            {gameState === "playing" && (
              <button
                ...
                className="flex items-center gap-1 bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-2 px-3 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 text-sm"
              >
            ...
            <button
              ...
              className="flex items-center gap-1 bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-3 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 text-sm"
            >
      ...
      <div className="relative max-h-[85vh]">
        ...
        <div
          ref={containerRef}
          style={{ width: "100%", aspectRatio: "770/600" }}
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col justify-between p-6">
          ...
          <AnimatePresence mode="wait">
            {stats.judgement && gameState === "playing" && (
              <motion.div
        ...
        <AnimatePresence>
          {(gameState === "ended" || gameState === "stopped") && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              ...
              >
                <div className="text-center">
                  <h2 className="text-3xl font-bold text-slate-800 mb-2">
                    {gameState === "ended" ? "Song Complete!" : "Game Stopped"}
                  </h2>
            ...
            >
              <div className="text-center">
                ...
                <div className="text-slate-500 text-sm mt-2">
                  Initializing {selectedSong.title}
                </div>
              </div>
            </motion.div>
          )}
      ...
      <div className="mt-4 bg-white/70 backdrop-blur-md rounded-2xl p-4 shadow-lg border border-white/50">
        ...
        <div className="w-full bg-slate-200 rounded-full h-3 relative overflow-hidden">
          <motion.div
            ...
            className={`h-3 rounded-full bg-gradient-to-r ${selectedSong.color} relative`}
          >
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-blue-50 via-indigo-50 to-purple-50 flex flex-col items-center justify-center p-4">
      ...
      <footer className="text-xs text-slate-400 mt-6">
        {currentView === "menu"
          ? "🎵 Select a song to begin your rhythm journey"
