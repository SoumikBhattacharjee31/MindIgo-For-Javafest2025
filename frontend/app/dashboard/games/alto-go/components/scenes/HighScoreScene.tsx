...
import {
  apiService,
  ScoreResponse,
} from "@/app/dashboard/games/alto-go/services/apiService";
...
export class HighScoresScene extends Scene {
  ...
  private createToggleButton() {
    ...
    this.toggleButton = this.add
      .text(width / 2, 120, "Show Personal Scores", {
        fontSize: "20px",
        color: "#ffffff",
    ...
    try {
      ...
      try {
        ...
        if (personalResponse.success && personalResponse.data) {
          this.personalScores = personalResponse.data
  ...
  private handleLoadingError(error: any) {
    ...
    // Show error message
    const errorMessage =
      error.message || "Failed to load scores. Please check your connection.";
  ...
  private loadOfflineScores() {
    try {
      ...
      dummyScores.forEach((score, index) => {
        if (score < highScore) {
          // Only add dummy scores lower than player's high score
          this.topScores.push({
            id: index + 2,
  ...
  private updateScoresDisplay() {
    ...
    const { width } = this.scale;
    const scores =
      this.currentView === "global" ? this.topScores : this.personalScores;
      ...
      const noScoresText = this.add
        .text(
          ...
          this.currentView === "global"
            ? "No global scores available"
            : "No personal scores found",
          {
            fontSize: "24px",
            color: "#cccccc",
          }
    ...
    scores.forEach((scoreData, index) => {
      ...
      const playerName =
        scoreData.playerName || `Player ${scoreData.playerId.slice(-6)}`;
      ...
      const scoreText = this.add
        .text(width - 100, yPos, scoreData.score.toLocaleString(), {
          fontSize: "20px",
          color: index < 3 ? "#ffd700" : "#ffffff",
          fontStyle: "bold",
        })
