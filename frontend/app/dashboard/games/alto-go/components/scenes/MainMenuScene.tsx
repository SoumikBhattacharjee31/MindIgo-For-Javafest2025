...
export class MainMenuScene extends Scene {
  ...
  private createAnimatedTitle() {
    ...
    this.titleText = this.add
      .text(width / 2, height / 2 - 120, "SNOWBOARDER", {
        fontSize: "64px",
        color: "#ffffff",
        fontStyle: "bold",
        stroke: "#003366",
  ...
  private createSubtitle() {
    ...
    const subtitle = this.add
      .text(width / 2, height / 2 - 60, "Extreme Mountain Adventure", {
        fontSize: "24px",
        color: "#cccccc",
        fontStyle: "italic",
      })
  ...
  private createUserInfo() {
    ...
    this.personalBestText = this.add
      .text(20, 50, "Personal Best: Loading...", {
        fontSize: "14px",
        color: "#cccccc",
    ...
    const accountButton = this.add
      .text(width - 20, 50, "Account", {
        fontSize: "16px",
        color: "#ffffff",
    ...
    this.setupButtonEffects(accountButton, "#2196F3", "#1976D2", () =>
  ...
  private createMenuButtons() {
    ...
    buttonConfigs.forEach((config, index) => {
      const button = this.add
        .text(width / 2, config.y, config.text, {
          fontSize: "32px",
          color: "#ffffff",
  ...
  private async loadUserStats() {
    try {
      ...
      if (response.success) {
        ...
        this.connectionStatusText.setStyle({
          color: "#00ff00",
        ...
        if (response.data && response.data.score) {
          this.personalBestText.setText(
            `Personal Best: ${response.data.score.toLocaleString()}`
          );
        } else {
  ...
  private showAccountMenu() {
    ...
    const modalFrame = this.add.rectangle(
      width / 2,
      height / 2,
      340,
      240,
    ...
    const modalTitle = this.add
      .text(width / 2, height / 2 - 80, "Account", {
        fontSize: "28px",
        color: "#ffffff",
        fontStyle: "bold",
      })
    ...
    const userInfo = this.add
      .text(width / 2, height / 2 - 40, `Logged in as:\n${userEmail}`, {
        fontSize: "16px",
        color: "#cccccc",
        align: "center",
      })
    ...
    const changeNameButton = this.add
      .text(width / 2, height / 2 + 10, "Change Name", {
        fontSize: "18px",
        color: "#ffffff",
    ...
    const logoutButton = this.add
      .text(width / 2, height / 2 + 50, "Logout", {
        fontSize: "18px",
        color: "#ffffff",
    ...
    const closeButton = this.add
      .text(width / 2, height / 2 + 90, "Close", {
        fontSize: "16px",
        color: "#ffffff",
  ...
  private showChangeNameDialog() {
    ...
    const modalTitle = this.add
      .text(width / 2, height / 2 - 60, "Change Name", {
        fontSize: "24px",
        color: "#ffffff",
        fontStyle: "bold",
      })
    ...
    const inputText = this.add
      .text(width / 2, height / 2 - 20, this.currentUser?.name || "", {
        fontSize: "18px",
        color: "#ffffff",
      })
      .setOrigin(0.5);
    ...
    const saveButton = this.add
      .text(width / 2 - 50, height / 2 + 40, "Save", {
        fontSize: "18px",
        color: "#ffffff",
        backgroundColor: "#4CAF50",
        padding: { x: 15, y: 8 },
    ...
    const cancelButton = this.add
      .text(width / 2 + 50, height / 2 + 40, "Cancel", {
        fontSize: "18px",
        color: "#ffffff",
  ...
  private showSettingsModal() {
    ...
    const musicToggle = this.add
      .text(width / 2, height / 2 - 30, "🔊 Music: ON", {
        fontSize: "24px",
        color: "#ffffff",
        backgroundColor: "#4CAF50",
        padding: { x: 15, y: 8 },
    ...
    musicToggle.on("pointerdown", () => {
      ...
      musicToggle.setStyle({
        backgroundColor: musicOn ? "#4CAF50" : "#F44336",
      });
    });

    const closeButton = this.add
      .text(width / 2, height / 2 + 60, "Close", {
        fontSize: "24px",
        color: "#ffffff",
        backgroundColor: "#607D8B",
        padding: { x: 20, y: 10 },
