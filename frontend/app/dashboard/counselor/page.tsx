  ...
  <Link href={`/dashboard/counselor/${counselor.id}`} className="block h-full">
    <div className="card bg-base-100 shadow-lg hover:shadow-xl transition-all duration-300 border border-base-200 rounded-2xl overflow-hidden h-full flex flex-col group">
      <div className="relative bg-gradient-to-b from-primary/10 to-transparent p-6 pt-8 pb-4 text-center">
        <img 
          src={counselor.profileImageUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(counselor.name)}&background=random&size=256`} 
      ...
      <div className="card-body p-6 flex-grow flex flex-col justify-between">
        <div>
          <h3 className="card-title text-xl font-semibold mb-1 line-clamp-1">{counselor.name}</h3>
          <p className="text-sm text-accent mb-3 line-clamp-1">{counselor.specialization}</p>
        </div>
...
const Pagination = ({ currentPage, totalPages, onPageChange }: { currentPage: number, totalPages: number, onPageChange: (page: number) => void }) => {
  ...
  return (
    <div className="join mt-10 justify-center flex flex-wrap gap-1">
      <button 
        className={`join-item btn btn-sm ${currentPage === 0 ? 'btn-disabled' : ''}`}
      ...
      {getPageNumbers().map((page, idx) => 
        typeof page === 'number' ? (
          <button 
            key={idx}
            className={`join-item btn btn-sm ${currentPage === page - 1 ? 'btn-active btn-primary text-white' : ''}`}
            onClick={() => onPageChange(page - 1)}
          >
            {page}
          </button>
        ) : (
          <span key={idx} className="join-item btn btn-sm btn-disabled">...</span>
        )
      )}

      <button 
        className={`join-item btn btn-sm ${currentPage === totalPages - 1 ? 'btn-disabled' : ''}`}
        onClick={() => onPageChange(currentPage + 1)}
...
const LoadingSkeleton = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
    {Array.from({ length: 9 }).map((_, i) => (
      <div key={i} className="card bg-base-200 shadow animate-pulse rounded-2xl border border-base-200 h-80 flex flex-col">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <header className="text-center mb-12">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Find Your Counselor</h1>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex-grow min-w-[250px]">
            <form onSubmit={handleSearchSubmit} className="relative">
              ...
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 btn btn-ghost btn-sm">
                Go
              </button>
            </form>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 px-6 bg-base-100 rounded-2xl">
          ...
          <p className="text-base-content/70 mb-6 max-w-md mx-auto">
            Try adjusting your filters or search terms. We’re sure you’ll find the perfect match!
          </p>
          <button 
            onClick={resetFilters} 
