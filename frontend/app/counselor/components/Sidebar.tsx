// app/counselor/components/Sidebar.tsx
...
import { User, House, Users2, Calendar, Video } from "lucide-react";
...
const SIDEBAR_ITEMS: SidebarItem[] = [
  ...
  },
  {
    ...
    link: "/counselor/discussion",
  },
  {
    id: "meeting",
    label: "Meeting",
    icon: <Video className="w-5 h-5" />,
    link: "/counselor/meeting",
  },
  {
    id: "profile",
    label: "Profile",
...
const styles = {
  ...
  navButton:
    "w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 ease-out relative overflow-hidden",
  activeIndicator:
    "absolute -left-1 top-1/2 transform -translate-y-1/2 w-1 h-8 bg-gradient-to-b from-indigo-500 to-blue-500 rounded-r-full shadow-sm animate-pulse",
  rippleEffect:
    "absolute inset-0 bg-gradient-to-r from-indigo-300/20 to-blue-300/20 rounded-full scale-0 group-hover:scale-100 transition-transform duration-500 ease-out",
  tooltip:
    "absolute left-full ml-4 px-3 py-2 bg-gray-900/90 backdrop-blur-sm text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-50 shadow-lg transform translate-x-2 group-hover:translate-x-0",
  tooltipArrow:
    "absolute -left-1 top-1/2 transform -translate-y-1/2 w-2 h-2 bg-gray-900/90 rotate-45",

  // Profile styles
  profileContainer: "flex relative group mt-auto",
  profileButton:
    "w-14 h-14 flex items-center justify-center rounded-full transition-all duration-500 ease-out relative overflow-hidden border-2",
} as const;
