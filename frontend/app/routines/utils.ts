...
const validateActivity = (
  ...
  if (!activity.activityName?.trim()) {
    errors.push({
      field: `activities[${index}].activityName`,
  ...
  if (!activity.activityType) {
    errors.push({
      field: `activities[${index}].activityType`,
  ...
  if (!activity.startTime) {
    errors.push({
      field: `activities[${index}].startTime`,
    ...
    if (currentEnd > nextStart) {
      errors.push({
        ...
        message: `Time overlap detected ${
          day !== "daily" ? `on ${day}` : ""
        } between "${current.activityName}" (${current.startTime}-${
          current.endTime
        }) and "${next.activityName}" (${next.startTime}-${next.endTime})`,
      });
    }
  }
