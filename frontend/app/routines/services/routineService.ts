...
import { CreateRoutineRequest, RoutineResponse, AssignRoutineRequest } from '../types/routine';
...
export const routineService = {
  ...
  createRoutine: async (request: CreateRoutineRequest): Promise<RoutineResponse> => {
    const response = await api.post<RoutineResponse>('/', request);
    return response.data;
  },

  // Update an existing routine
  updateRoutine: async (routineId: number, request: CreateRoutineRequest, doctorId: number): Promise<RoutineResponse> => {
  ...
  assignRoutineToPatient: async (request: AssignRoutineRequest): Promise<void> => {
    await api.post('/assign', request);
  },

  // Unassign routine from patient
  unassignRoutineFromPatient: async (patientId: number, routineId: number): Promise<void> => {
