...
import QuizCard from "@/app/admin/components/QuizCard";
...
const QuizOverview = () => {
  ...
  const [filterBy, setFilterBy] = useState<"all" | "completed" | "empty">(
    "all"
  );
  ...
  const handleUpdateAnalysisLink = async (
    userId: string,
    quizCode: string,
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 400) {
          ...
          errorToast(
            error.response?.data?.message || "Failed to update analysis link"
          );
        }
      } else {
        errorToast("Network error occurred");
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage existing quizzes and view user responses
          </p>
        </div>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div>
            ...
            <select
              value={filterBy}
              onChange={(e) =>
                setFilterBy(e.target.value as "all" | "completed" | "empty")
              }
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {quizzes.reduce(
                  (total, quiz) => total + quiz.completedUsers.length,
                  0
                )}
              </p>
            </div>
