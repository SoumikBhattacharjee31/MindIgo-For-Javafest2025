...
const ReportsTable = ({ 
  ...
  const formatReason = (reason: string) => {
    return reason.toLowerCase().replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  };
  ...
  if (reports.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-500">No {type} reports to review at this time.</p>
      </div>
    );
  }
            ...
            <tr key={report.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900">
                  {type === 'post' ? `Post #${report.postId}` : `Comment #${report.commentId}`}
                </div>
                {report.description && (
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-900">{report.reporterEmail}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getReasonColor(report.reason)}`}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                {report.status === 'PENDING' && (
                  <>
                    ...
                    <button
                      onClick={() => onRestrictUser(report.authorId, report.authorEmail)}
