...
import { PostResponse, discussionApi, REACTION_TYPES, POST_CATEGORIES } from '../../api/discussionService';
...
const PostCard = ({ 
  ...
  const handleReaction = async (reactionType: string) => {
    try {
      const response = await discussionApi.reactToPost(post.id, { reactionType });
  ...
  const handleReport = async (reason: string, description?: string) => {
    try {
      const response = await discussionApi.reportPost(post.id, { reason, description });
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getRoleColor(post.authorRole)}`}>
                  {post.authorRole}
                </span>
              </div>
