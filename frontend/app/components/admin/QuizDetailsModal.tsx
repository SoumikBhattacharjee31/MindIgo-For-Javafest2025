        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-600 mb-2">Quiz Code</h3>
              <div className="flex items-center space-x-2">
                <span className="text-lg font-mono text-gray-900">{quiz.quizCode}</span>
                <button
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-600 mb-2">File ID</h3>
              <span className="text-lg text-gray-900">{quiz.fileId}</span>
            </div>

            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-600 mb-2">Responses</h3>
              <span className="text-lg text-gray-900">{quiz.completedUsers.length}</span>
            </div>
          </div>
                  ...
                  <div key={question.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        {question.type.toLowerCase() === 'scale' && question.scaleLabels && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {Object.entries(question.scaleLabels).map(([value, label]) => (
                                <div key={value} className="flex items-center space-x-1 bg-gray-100 px-2 py-1 rounded">
                        ...
                        {question.type.toLowerCase() === 'descriptive' && (
                          <div>
                            <h4 className="text-sm font-medium text-gray-600">Response Type:</h4>
                            <span className="text-sm text-gray-500">Open-ended text response</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                {quiz.completedUsers.map((userId) => (
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-700 truncate">{userId}</span>
                  </div>
                ))}
              </div>
            </div>
